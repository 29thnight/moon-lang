# PrSM Language Specification — v0 Design Package
...
PrSM is a **Unity-first scripting language** with Kotlin-flavored syntax. It exists to make Unity gameplay programming **safer, more concise, and less ceremonial** than raw C# while remaining a **zero-cost abstraction** — every PrSM program lowers to idiomatic C# that Unity developers could have written by hand.

**Core design principles:**

1. **Unity is the runtime.** PrSM does not define its own runtime, GC, or object model. Unity's MonoBehaviour lifecycle, serialization, and ECS conventions are first-class language constructs, not library wrappers.
...
4. **Transparent lowering.** A PrSM author can predict the C# output. No hidden allocations, no implicit coroutine state machines beyond what Unity already uses, no reflection at runtime.
...
| Non-goal | Rationale |
|---|---|
| General-purpose language | PrSM targets Unity scripting only. It is not a replacement for C# outside Unity. |
...
| Source-level C# interop (embedding C# in PrSM) | PrSM files are standalone. C# interop is assembly-level: PrSM calls C# types, C# calls generated PrSM types. |
...
PrSM uses **significant braces** (not indentation), **no semicolons** (newline is statement terminator; line continuation via trailing operator or `\`), and **Kotlin-style declarations**.
...
**Semantics:**
A `component` is the primary PrSM declaration. It represents a Unity MonoBehaviour. There is no explicit base class — all components implicitly extend `MonoBehaviour`.
...
| PrSM block | Unity method | Signature | Timing |
|---|---|---|---|
...
**Semantics:**
All types are **non-null by default**. Appending `?` creates the nullable variant. This is the core safety guarantee of PrSM.

**Lowering:**
At the C# level, nullable reference types may or may not be emitted depending on the target C# version. The PrSM compiler enforces null safety at compile time regardless of C# NRT support. For value types, `T?` lowers to `Nullable<T>`.

**Unity special case — UnityEngine.Object null:**
Unity has a custom `== null` that returns true for destroyed objects. PrSM respects this:
...
Note: PrSM parameter names that are C# keywords get `@` prefix in generated code.
...
**Semantics:**
`when` is PrSM's pattern matching construct (Kotlin-style). Subjects are matched top-to-bottom. First match wins.
...
PrSM supports calling generic Unity APIs:
...
**v0 scope:** PrSM code can CALL generic methods and use generic types from C#. PrSM does NOT support declaring new generic types or functions in v0. Deferred.

### 10.4 Using C# Types from PrSM

Any C# type visible to the Unity compilation pipeline is usable in PrSM. No import statement is needed for `UnityEngine` types. For other namespaces:
...
**`using` statements** appear at the top of a PrSM file, before declarations.

### 10.5 Using PrSM Types from C#

Generated C# classes are public and follow C# naming conventions. A C# script can reference a PrSM-generated type directly:

```csharp
// In a C# file
Player player = GetComponent<Player>(); // Player was defined in PrSM
player.health -= 10;
```
...
**Lowering:** `new Vector3(1, 2, 3)` — the `new` keyword is implicit in PrSM.
...
| C# calling PrSM types | Included |
| Declaring generics in PrSM | Deferred |
| Custom Unity Editor scripts in PrSM | Deferred |
| Unity DOTS/ECS integration | Deferred |
| NativeArray/NativeSlice/Jobs | Deferred |
| Custom PropertyDrawers in PrSM | Deferred |
...
Each `.prsm` file produces exactly one `.cs` file in a mirror directory structure under a generated folder (e.g., `Assets/Generated/PrSM/`).

```
Assets/Scripts/Player.prsm → Assets/Generated/PrSM/Player.cs
Assets/Scripts/Enemies/Goblin.prsm → Assets/Generated/PrSM/Enemies/Goblin.cs
```
...
The PrSM compiler erases most null-safety information during lowering — it exists only at the PrSM type-checking layer. The generated C# uses nullable reference type annotations where available (`#nullable enable`) as documentation, but correctness is guaranteed by the PrSM compiler, not C# NRT.
...
```csharp
// <auto-generated>
// This file was generated by the PrSM compiler. Do not edit manually.
// Source: Player.prsm
// </auto-generated>
```
...
- [ ] All type references resolve to a known type (PrSM-defined, C# imported, or UnityEngine).
...
component Hero { }
```
**Diagnostic:** `E071: Declaration name 'Hero' does not match file name 'Player.prsm'. Rename the declaration or the file.`
...
*End of PrSM Language v0 Specification*
