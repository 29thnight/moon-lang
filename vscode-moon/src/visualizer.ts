...
/**
 * Moon Script Visualizer — shows script structure as an interactive diagram
 * in a right-side split panel (WebView).
 */
export class MoonVisualizer {

    private static panel: vscode.WebviewPanel | undefined;

    static show(document: vscode.TextDocument) {
        const text = document.getText();
        const fileName = path.basename(document.uri.fsPath, '.mn');
        const structure = parseForVisualization(text, fileName);

        if (MoonVisualizer.panel) {
            MoonVisualizer.panel.webview.html = generateHtml(structure, fileName);
            MoonVisualizer.panel.reveal(vscode.ViewColumn.Beside, true);
        } else {
            MoonVisualizer.panel = vscode.window.createWebviewPanel(
                'moonVisualizer',
                `Moon: ${fileName}`,
            ...
            MoonVisualizer.panel.webview.html = generateHtml(structure, fileName);
            MoonVisualizer.panel.onDidDispose(() => { MoonVisualizer.panel = undefined; });
        }
    }

    static dispose() {
        MoonVisualizer.panel?.dispose();
    }
}
