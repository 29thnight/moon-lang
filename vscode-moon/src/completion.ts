...
import { TYPE_MAP, NAMESPACE_TYPES, MOON_KEYWORDS, MOON_BUILTINS, UnityMember } from './unity-api';
 ...
 * 2. User scripts: regex scan of workspace .mn files
 */
export class MoonCompletionProvider implements vscode.CompletionItemProvider {
    ...
    constructor(extensionPath: string) {
        ...
        if (this.dbReady) {
            console.log('Moon: Unity API DB loaded from', dbPath);
        } else {
            console.log('Moon: Unity API DB not found at', dbPath, '— using hardcoded fallback');
        }

        vscode.workspace.onDidSaveTextDocument(doc => {
            if (doc.languageId === 'moon') { this.symbolsStale = true; }
        });
    ...
    private async getMemberCompletions(document: vscode.TextDocument, receiver: string): Promise<vscode.CompletionItem[]> {
        ...
        try {
            const csharpItems = await this.csharp.getCompletionsFromCSharp(typeName, '');
            if (csharpItems.length > 0) {
                return filterCompletions(csharpItems);
            }

            const unityItems = await this.csharp.getUnityTypeMembers(typeName);
    ...
    private getKeywordCompletions(): vscode.CompletionItem[] {
        return MOON_KEYWORDS.map(kw => {
            const item = new vscode.CompletionItem(kw, vscode.CompletionItemKind.Keyword);
            item.detail = 'Keyword (moon)';
            return item;
        });
    }

    private getBuiltinCompletions(): vscode.CompletionItem[] {
        return MOON_BUILTINS.map(b => {
            const item = new vscode.CompletionItem(b.name, vscode.CompletionItemKind.Function);
            item.detail = b.moonOnly ? 'Builtin (moon)' : 'Builtin';
            const md = new vscode.MarkdownString();
            md.appendCodeblock(`${b.name}${b.params}`, 'moon');
        ...
        for (const [, symbols] of this.userSymbols) {
            for (const sym of symbols) {
                ...
                const kindLabels: Record<string, string> = {
                    component: 'Component (moon)', asset: 'Asset (moon)', func: 'Method (moon)',
                    field: 'Field (moon)', coroutine: 'Coroutine (moon)', enum: 'Enum (moon)', enumEntry: 'EnumMember (moon)'
                };
    ...
    private async scanWorkspaceSymbols() {
        ...
        const files = await vscode.workspace.findFiles('**/*.mn', '**/node_modules/**');
