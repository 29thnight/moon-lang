...
function sampleSourceMap(sourceFile: string, generatedFile: string): MoonGeneratedSourceMap {
    return {
        version: 1,
...
test('sourceMapPathForGeneratedFile uses the mnmap sidecar extension', () => {
    assert.equal(
        sourceMapPathForGeneratedFile(path.join('Generated', 'Player.cs')),
        path.join('Generated', 'Player.mnmap.json'),
    );
});

test('resolveGeneratedSourceMapPath finds a sidecar next to configured generated output', () => {
    const root = tempDir('moon-source-map-path-');
    const moonFile = path.join(root, 'Assets', 'Scripts', 'Player.mn');
    const generatedDir = path.join(root, 'Generated', 'Moon');
    const generatedFile = path.join(generatedDir, 'Player.cs');
    const sourceMapFile = path.join(generatedDir, 'Player.mnmap.json');

    fs.mkdirSync(path.dirname(moonFile), { recursive: true });
    fs.mkdirSync(generatedDir, { recursive: true });
    fs.writeFileSync(path.join(root, '.mnproject'), `[project]\nname = "Test"\n\n[compiler]\noutput_dir = "Generated/Moon"\n`);
    fs.writeFileSync(moonFile, 'component Player : MonoBehaviour {}');
    fs.writeFileSync(generatedFile, '// generated');
    fs.writeFileSync(sourceMapFile, JSON.stringify(sampleSourceMap(moonFile, generatedFile)));

    assert.equal(resolveGeneratedSourceMapPath(moonFile, [root]), sourceMapFile);
...
test('source map helpers map source and generated positions through anchors', () => {
    const sourceMap = sampleSourceMap('/workspace/Assets/Player.mn', '/workspace/Generated/Player.cs');
...
test('resolveSourceMapSourcePath resolves relative source files against the project root', () => {
    const root = tempDir('moon-source-map-source-path-');
    const sourceFile = path.join(root, 'Assets', 'Player.mn');
    const generatedFile = path.join(root, 'Generated', 'Moon', 'Player.cs');
    ...
    const resolved = resolveSourceMapSourcePath(generatedFile, sampleSourceMap(path.join('Assets', 'Player.mn'), generatedFile), [root]);
