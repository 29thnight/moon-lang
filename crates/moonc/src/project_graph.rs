...
impl ProjectGraph {
    pub fn discover(start_dir: &Path) -> Result<Self, String> {
        let (config, project_root) = MoonProject::find_and_load(start_dir)?;
        ...
        let cache_dir = project_root.join(".moon").join("cache");
...
fn collect_watch_roots(config: &MoonProject, project_root: &Path) -> Vec<PathBuf> {
...
mod tests {
    ...
    fn discover_collects_language_and_unity_capabilities() {
        let root = unique_temp_dir("moonc_project_graph");
        write_file(
            &root.join(".mnproject"),
            r#"[project]
name = "GraphProject"
...
"#,
  ...
  "dependencies": {
    "com.unity.inputsystem": "1.8.1"
        ...
        assert_eq!(graph.output_dir, root.join("Generated").join("Moon"));
        assert_eq!(graph.cache_dir, root.join(".moon").join("cache"));
        assert_eq!(graph.source_files, vec![root.join("Assets").join("Hero.mn")]);
    ...
    fn discover_rejects_unknown_language_feature() {
        let root = unique_temp_dir("moonc_project_graph_feature_error");
        write_file(
            &root.join(".mnproject"),
            r#"[project]
name = "GraphProject"
...
"#,
    ...
    fn discover_applies_defaults_when_sections_are_omitted() {
        let root = unique_temp_dir("moonc_project_graph_defaults");
        write_file(
            &root.join(".mnproject"),
            r#"[project]
...
"#,
        ...
        assert_eq!(graph.output_dir, root.join("Assets").join("Generated").join("Moon"));
        assert_eq!(graph.source_files, vec![root.join("Scene.mn")]);
