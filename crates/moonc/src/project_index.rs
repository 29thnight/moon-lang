...
mod tests {
    ...
    fn build_project_index_collects_files_and_symbols() {
        let root = unique_temp_dir("moonc_project_index");
        let player = root.join("Player.mn");
        let state = root.join("EnemyState.mn");
...
"#,
    ...
    fn build_project_index_skips_invalid_files() {
        let root = unique_temp_dir("moonc_project_index_invalid");
        let broken = root.join("Broken.mn");
