...
pub fn source_map_path_for_generated(generated_file: &Path) -> PathBuf {
    let stem = generated_file
        ...
        .unwrap_or("generated");
    generated_file.with_file_name(format!("{}.mnmap.json", stem))
}
...
fn build_statement_segment(
    ...
    let (children, next_line) = match statement {
        ...
        _ => (Vec::new(), start_line.saturating_add(1)),
    };
...
mod tests {
    ...
    use crate::semantic::types::{MoonType, PrimitiveKind};

    #[test]
    fn source_map_path_uses_mnmap_extension() {
        let path = source_map_path_for_generated(Path::new("Generated/Player.cs"));
        assert_eq!(path, PathBuf::from("Generated/Player.mnmap.json"));
    }

    #[test]
    fn build_source_map_tracks_declaration_and_members() {
        let hir_file = HirFile {
            path: PathBuf::from("Assets/Player.mn"),
            definitions: vec![
...
"#;
    ...
    fn definition(
        ...
        HirDefinition {
            ...
            kind,
            ty: MoonType::Primitive(PrimitiveKind::Int),
            mutable: false,
            file_path: PathBuf::from("Assets/Player.mn"),
            span,
        }
    }
