...
fn lower_decl(decl: &Decl) -> (CsClass, Vec<CsClass>) {
    match decl {
        Decl::Component { name, is_singleton, is_partial, base_class, interfaces, members, .. } => {
            ...
            // v2 New Input System sugar: basic input.action() uses the project-wide
            // InputSystem.actions API and does NOT require PlayerInput.  PlayerInput
            // infrastructure is only injected when @inputActions annotation is present
            // (explicit asset wiring).
            let input_actions_binding = collect_input_actions_binding(members);
            let needs_player_input = input_actions_binding.is_some();
...
fn lower_expr_with_expected_type(
    ...
    match expr {
        ...
        Expr::MemberAccess { receiver, name, .. } => {
            // New Input System sugar: input.action("X").pressed → InputSystem.actions.FindAction("X").WasPressedThisFrame()
        ...
        if name == "action" {
            if let Some(arg) = args.first() {
                if let Expr::StringLit(s, _) = &arg.value {
                    ...
                    // Chained form: input.map("Map").action("Name")
                    if extract_input_map_name(recv).is_some() {
                        return Some(s.as_str());
                    }
                }
            }
...
/// If `expr` is `input.map("MapName")`, return `Some(map_name)`.
fn extract_input_map_name(expr: &Expr) -> Option<&str> {
    if let Expr::Call { receiver: Some(recv), name, args, .. } = expr {
        if name == "map" || name == "player" {
            if let Expr::Ident(id, _) = recv.as_ref() {
                if id == "input" {
                    if let Some(arg) = args.first() {
                        if let Expr::StringLit(s, _) = &arg.value {
...
///
/// Uses Unity's project-wide Actions workflow (InputSystem.actions.FindAction).
///
/// Patterns handled (direct form), null-safe via `?.` and `?? default`:
/// - `input.action("Name").pressed`   → `(InputSystem.actions?.FindAction("Name")?.WasPressedThisFrame() ?? false)`
/// - `input.action("Name").released`  → `(InputSystem.actions?.FindAction("Name")?.WasReleasedThisFrame() ?? false)`
/// - `input.action("Name").held`      → `(InputSystem.actions?.FindAction("Name")?.IsPressed() ?? false)`
/// - `input.action("Name").vector2`   → `(InputSystem.actions?.FindAction("Name")?.ReadValue<Vector2>() ?? default)`
/// - `input.action("Name").scalar`    → `(InputSystem.actions?.FindAction("Name")?.ReadValue<float>() ?? 0f)`
/// - `input.action("Name").value`     → same as `vector2` (generic alias)
///
/// Map form (explicit action map):
/// - `input.map("Map").action("Name").pressed` → `(InputSystem.actions?.FindAction("Map/Name")?.WasPressedThisFrame() ?? false)`
fn try_lower_new_input_member(receiver: &Expr, prop: &str) -> Option<String> {
    let action_name = extract_input_action_name(receiver)?;

    // Determine the action lookup key. For the map form, prefix with the map name.
    let action_key = if let Expr::Call { receiver: Some(map_call), .. } = receiver {
        if let Some(map_name) = extract_input_map_name(map_call) {
    ...
    // Null-safe: InputSystem.actions or FindAction may be null when
    // project-wide actions are not configured or the action name is wrong.
    let find = format!("InputSystem.actions?.FindAction(\"{}\")", action_key);
    ...
    let cs = match prop {
        ...
        "vector2" | "value" => format!(
            "({}?.ReadValue<UnityEngine.Vector2>() ?? default)",
            find
        ),
        "scalar" => format!(
            "({}?.ReadValue<float>() ?? 0f)",
