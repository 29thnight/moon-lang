...
{
    [InitializeOnLoad]
    internal static class PrismRuntimeStackTraceRemapper
    {
        private static bool _isEmittingRemap;

        // Maps absolute .prsm source path → (line, col) for the most recently emitted remapped log.
        // Populated when the remapped log is emitted; consumed by PrismScriptProxy.OnOpenPrSMAsset
        ...
        static PrismRuntimeStackTraceRemapper()
        {
        ...
        private static void HandleLogMessage(string condition, string stackTrace, LogType type)
        {
            ...
            string remappedMessage = PrismStackTraceFormatter.FormatRemappedRuntimeMessage(
        ...
        /// <summary>
        /// Returns the cached (line, col) for a .prsm source file if it was populated from a remapped
        ...
        internal static bool TryResolveRemappedAssetPath(string projectRoot, string remappedMessage, out string assetPath)
        {
            ...
            if (!PrismStackTraceFormatter.TryExtractFirstPrSMLocation(remappedMessage, out string sourcePath, out _, out _))
            {
                return false;
            }
