...
{
    [Serializable]
    internal class PrSMGeneratedSourceMapFile
    {
        public int version;
        public string source_file = string.Empty;
        public string generated_file = string.Empty;
        public PrSMGeneratedSourceMapAnchor declaration;
        public PrSMGeneratedSourceMapAnchor[] members = Array.Empty<PrSMGeneratedSourceMapAnchor>();
    }

    [Serializable]
    internal class PrSMGeneratedSourceMapAnchor
    {
        public string kind = string.Empty;
        public string name = string.Empty;
        public string qualified_name = string.Empty;
        public PrismGeneratedSourceMapSpan source_span;
        public PrismGeneratedSourceMapSpan generated_span;
        public PrismGeneratedSourceMapSpan generated_name_span;
        public PrSMGeneratedSourceMapAnchor[] segments = Array.Empty<PrSMGeneratedSourceMapAnchor>();
    }

    [Serializable]
    internal class PrismGeneratedSourceMapSpan
    {
        public int line;
        public int col;
        public int end_line;
    ...
    internal static class PrismSourceMap
    {
        internal static string GetSourceMapPath(string generatedFilePath)
        {
            ...
            return Path.Combine(directory, stem + ".prsmmap.json");
        }

        internal static PrSMGeneratedSourceMapFile LoadSourceMap(string generatedFilePath)
        {
            ...
            try
            {
                return PrSMSourceMapJsonParser.Parse(File.ReadAllText(sourceMapPath));
            }
            catch
            {
                return null;
            ...
            out int sourceCol)
        {
            ...
            PrSMGeneratedSourceMapFile sourceMap = LoadSourceMap(generatedFilePath);
            if (sourceMap == null)
            {
                return false;
            }

            PrSMGeneratedSourceMapAnchor anchor = FindAnchorForGeneratedPosition(sourceMap, generatedLine, generatedCol);
        ...
        internal static PrSMGeneratedSourceMapAnchor FindAnchorForGeneratedPosition(
            PrSMGeneratedSourceMapFile sourceMap,
            int generatedLine,
            int generatedCol)
        {
            PrSMGeneratedSourceMapAnchor anchor = FindMostSpecificAnchor(
        ...
        private static IEnumerable<PrSMGeneratedSourceMapAnchor> GetAnchors(PrSMGeneratedSourceMapFile sourceMap)
        {
            if (sourceMap == null)
            {
                yield break;
            ...
            {
                foreach (PrSMGeneratedSourceMapAnchor anchor in EnumerateAnchor(sourceMap.declaration))
                {
                    yield return anchor;
                }
            }

            foreach (PrSMGeneratedSourceMapAnchor member in sourceMap.members ?? Array.Empty<PrSMGeneratedSourceMapAnchor>())
            {
                if (member != null)
                {
                    foreach (PrSMGeneratedSourceMapAnchor anchor in EnumerateAnchor(member))
                    {
                        yield return anchor;
                    }
                }
        ...
        private static IEnumerable<PrSMGeneratedSourceMapAnchor> EnumerateAnchor(PrSMGeneratedSourceMapAnchor anchor)
        {
            if (anchor == null)
            {
                yield break;
            ...
            foreach (PrSMGeneratedSourceMapAnchor segment in anchor.segments ?? Array.Empty<PrSMGeneratedSourceMapAnchor>())
            {
                foreach (PrSMGeneratedSourceMapAnchor child in EnumerateAnchor(segment))
                {
                    yield return child;
                }
            }
        ...
        private static PrSMGeneratedSourceMapAnchor FindMostSpecificAnchor(
            IEnumerable<PrSMGeneratedSourceMapAnchor> anchors,
            int line,
            int col,
            bool useGeneratedNameSpan)
        {
            PrSMGeneratedSourceMapAnchor bestAnchor = null;
            int bestSize = int.MaxValue;

            foreach (PrSMGeneratedSourceMapAnchor anchor in anchors)
            {
                PrismGeneratedSourceMapSpan span = useGeneratedNameSpan ? anchor.generated_name_span : anchor.generated_span;
        ...
        private static bool ContainsSpan(PrismGeneratedSourceMapSpan span, int line, int col)
        {
            if (span == null)
            {
                return false;
        ...
        private static int GetSpanSize(PrismGeneratedSourceMapSpan span)
        {
            if (span == null)
            {
                return int.MaxValue;
    ...
    internal static class PrSMSourceMapJsonParser
    {
        internal static PrSMGeneratedSourceMapFile Parse(string text)
        {
            var parser = new JsonParser(text);
            return ReadSourceMapFile(parser.ParseObject());
        }

        private static PrSMGeneratedSourceMapFile ReadSourceMapFile(Dictionary<string, object> data)
        {
            return new PrSMGeneratedSourceMapFile
            {
        ...
        private static PrSMGeneratedSourceMapAnchor ReadAnchor(Dictionary<string, object> data, string key)
        {
            return ReadAnchor(ReadObject(data, key));
        }

        private static PrSMGeneratedSourceMapAnchor ReadAnchor(Dictionary<string, object> data)
        {
            if (data == null)
            {
                return null;
            ...
            return new PrSMGeneratedSourceMapAnchor
            {
        ...
        private static PrSMGeneratedSourceMapAnchor[] ReadAnchorArray(Dictionary<string, object> data, string key)
        {
            List<object> values = ReadArray(data, key);
            if (values == null || values.Count == 0)
            {
                return Array.Empty<PrSMGeneratedSourceMapAnchor>();
            }

            var anchors = new List<PrSMGeneratedSourceMapAnchor>(values.Count);
            foreach (object value in values)
            {
                PrSMGeneratedSourceMapAnchor anchor = ReadAnchor(value as Dictionary<string, object>);
                if (anchor != null)
                {
                    anchors.Add(anchor);
                }
        ...
        private static PrismGeneratedSourceMapSpan ReadSpan(Dictionary<string, object> data, string key)
        {
            ...
            return new PrismGeneratedSourceMapSpan
            {
