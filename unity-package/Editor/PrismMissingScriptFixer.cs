...
namespace Prism.Editor
{
    ...
    /// </summary>
    [InitializeOnLoad]
    public static class PrismMissingScriptFixer
    {
        static PrismMissingScriptFixer()
        {
        ...
        private static void OnHeaderGUI(UnityEditor.Editor editor)
        {
            ...
            if (GUILayout.Button(label, style))
            {
                ...
                Debug.Log($"[PrSM] Removed {removed} missing script(s) from {go.name}");
            }

            GUI.backgroundColor = prev;
        }

        [MenuItem("GameObject/PrSM/Clean Up Missing Scripts", false, 50)]
        ...
        [MenuItem("GameObject/PrSM/Clean Up Missing Scripts", true)]
