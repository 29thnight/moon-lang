...
import type { PrismCSharpHoverInfo, PrismCSharpLookupTarget } from './csharp-navigation';
import { findGeneratedSpanForTarget, readGeneratedSourceMap } from './generated-source-map';
import { findPrismProjectRoot, getOutputDirCandidates, readPrismProject } from './project-config';

/**
 * Bridge to C# extension (OmniSharp / C# Dev Kit).
 * Queries the generated .cs files for type information,
 * then feeds results back to PrSM's CompletionProvider.
 */
export class CSharpBridge {
    private readonly outputDirCache = new Map<string, string>();

    async getHoverDetails(target: PrismCSharpLookupTarget, resourcePath?: string): Promise<PrismCSharpHoverInfo | null> {
        const outputDir = this.getOutputDir(resourcePath);
        if (!outputDir) {
            return null;
        }

        const generatedFile = this.getGeneratedFilePath(target.typeName, outputDir);
        const hoverText = target.memberName
            ? await this.getMemberHoverFromCSharp(target.typeName, target.memberName, outputDir, generatedFile)
            : await this.getTypeHoverFromCSharp(target.typeName, outputDir, generatedFile);
    ...
    async getCompletionsFromCSharp(
        ...
        const csFile = this.getGeneratedFilePath(typeName, outputDir);
    ...
    async getHoverFromCSharp(
        ...
        return this.getMemberHoverFromCSharp(
            typeName,
            memberName,
            outputDir,
            this.getGeneratedFilePath(typeName, outputDir),
        );
    }
     ...
     */
    async getUnityTypeMembers(typeName: string, resourcePath?: string): Promise<vscode.CompletionItem[]> {
        const outputDir = this.getOutputDir(resourcePath);
        if (!outputDir) return [];
        ...
        const csFiles = fs.readdirSync(outputDir).filter(f => f.endsWith('.cs'));
        if (csFiles.length === 0) return [];

        const csFile = path.join(outputDir, csFiles[0]);
    ...
    private getGeneratedFilePath(typeName: string, outputDir: string): string | undefined {
        const candidate = path.join(outputDir, `${typeName}.cs`);
        return fs.existsSync(candidate) ? candidate : undefined;
    }

    private async getTypeHoverFromCSharp(
        typeName: string,
        outputDir: string,
        ...
        for (const csFile of this.getHoverSearchFiles(outputDir, generatedFile)) {
            try {
                const csUri = vscode.Uri.file(csFile);
                const csDoc = await vscode.workspace.openTextDocument(csUri);
    ...
    private async getMemberHoverFromCSharp(
        ...
        const csFile = generatedFile ?? this.getGeneratedFilePath(typeName, outputDir);
        if (!csFile) {
            return undefined;
        }
    ...
    private getHoverSearchFiles(outputDir: string, preferredFile?: string): string[] {
    ...
    private getOutputDir(resourcePath?: string): string | undefined {
        for (const root of this.getProjectRoots(resourcePath)) {
            const cached = this.outputDirCache.get(root);
            ...
            if (resolved) {
                this.outputDirCache.set(root, resolved);
                return resolved;
            }
        }
    ...
    private getProjectRoots(resourcePath?: string): string[] {
        const roots = new Set<string>();

        if (resourcePath) {
            const projectRoot = findPrismProjectRoot(resourcePath);
            if (projectRoot) {
