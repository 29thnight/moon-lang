...
/**
 * PrSM Scripts sidebar TreeView.
 * Shows all .prsm files in the workspace grouped by folder.
 */
export class PrismExplorerProvider implements vscode.TreeDataProvider<PrSMTreeItem> {

    private _onDidChangeTreeData = new vscode.EventEmitter<PrSMTreeItem | undefined>();
    ...
    getTreeItem(element: PrSMTreeItem): vscode.TreeItem {
        return element;
    }

    async getChildren(element?: PrSMTreeItem): Promise<PrSMTreeItem[]> {
        if (!element) {
    ...
    private async getRootItems(): Promise<PrSMTreeItem[]> {
        let files: vscode.Uri[] = [];

        // 1. Try workspace
        if (vscode.workspace.workspaceFolders) {
            files = await vscode.workspace.findFiles('**/*.prsm', '**/node_modules/**');
        }
        ...
        if (groups.size === 1) {
            ...
            for (const [folder, uris] of [...groups.entries()].sort()) {
                ...
                const folderItem = new PrSMTreeItem(
                    folder,
    ...
    private createFileItem(uri: vscode.Uri): PrSMTreeItem {
        const name = path.basename(uri.fsPath, '.prsm');
        const item = new PrSMTreeItem(name, vscode.TreeItemCollapsibleState.Collapsed);
    ...
    private getFileStructure(filePath: string): PrSMTreeItem[] {
        try {
    ...
    private async scanDirectory(dir: string): Promise<vscode.Uri[]> {
        ...
        // Walk up to find project root (.prsmproject)
        let root = dir;
        for (let i = 0; i < 10; i++) {
            if (fs.existsSync(path.join(root, '.prsmproject'))) break;
            const parent = path.dirname(root);
            if (parent === root) break;
            root = parent;
        }
        // Recursively find .prsm files
    ...
    private findMnFiles(dir: string, results: vscode.Uri[]) {
        try {
            ...
            for (const entry of entries) {
                ...
                } else if (entry.name.endsWith('.prsm')) {
    ...
    private parseStructure(text: string): PrSMTreeItem[] {
        ...
        while ((fm = fieldRegex.exec(text)) !== null) {
            const fieldItem = new PrSMTreeItem(fm[2], vscode.TreeItemCollapsibleState.None);
        ...
        while ((funcM = funcRegex.exec(text)) !== null) {
            const funcItem = new PrSMTreeItem(funcM[1], vscode.TreeItemCollapsibleState.None);
        ...
        while ((corM = corRegex.exec(text)) !== null) {
            const corItem = new PrSMTreeItem(corM[1], vscode.TreeItemCollapsibleState.None);
        ...
        while ((lcM = lifecycleRegex.exec(text)) !== null) {
            const lcItem = new PrSMTreeItem(lcM[1], vscode.TreeItemCollapsibleState.None);
...
export class PrSMTreeItem extends vscode.TreeItem {
    children?: PrSMTreeItem[];
}
