...
export class PrismGraphView {
    private static panel: vscode.WebviewPanel | undefined;
    static async show(context: vscode.ExtensionContext) {
        _extensionPath = context.extensionPath;
        const graph = await buildGraph();
        if (PrismGraphView.panel) {
            PrismGraphView.panel.webview.html = generateGraphHtml(graph);
            PrismGraphView.panel.reveal(vscode.ViewColumn.Beside, true);
        } else {
            PrismGraphView.panel = vscode.window.createWebviewPanel(
                'prsmGraphView', 'PrSM: Graph View',
                { viewColumn: vscode.ViewColumn.Beside, preserveFocus: true },
                { enableScripts: true }
            );
            PrismGraphView.panel.webview.html = generateGraphHtml(graph);
            PrismGraphView.panel.onDidDispose(() => { PrismGraphView.panel = undefined; });
            PrismGraphView.panel.webview.onDidReceiveMessage(async msg => {
                await PrismGraphView.handleMessage(msg);
            });
        }
    }
    ...
    static async handleMessage(msg: any) {
        if ((msg.type === 'openFile' || msg.type === 'visualize') && msg.file) {
            const uri = await PrismGraphView.resolveFile(msg.file);
            if (!uri) {
            ...
            if (msg.type === 'openFile') {
                ...
                const { PrismVisualizer } = require('./visualizer');
                PrismVisualizer.show(doc);
            }
        }
    }
    ...
    private static async resolveFile(filePath: string): Promise<vscode.Uri | null> {
        ...
        if (editor) {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                if (fs.existsSync(path.join(dir, '.prsmproject'))) break;
                const parent = path.dirname(dir);
                if (parent === dir) break;
                dir = parent;
            }
        }
        // 4. Scan for .prsm files
...
/** Parse a single .prsm file into a node + edges */
...
async function buildGraph(): Promise<Graph> {
    ...
    // Collect all .prsm files in workspace
    let allFiles: vscode.Uri[] = [];
    if (vscode.workspace.workspaceFolders)
        allFiles = await vscode.workspace.findFiles('**/*.prsm', '**/node_modules/**');
    ...
    if (activeEditor && activeEditor.document.fileName.endsWith('.prsm')) {
        const text = activeEditor.document.getText();
        const m = text.match(/\b(component|asset|class|enum)\s+(\w+)/);
        if (m) focusName = m[2];
    }

    // If no .prsm file is active, fall back to showing all
...
function findMnSync(dir: string): string[] {
    ...
    (function w(d: string) {
        try { for (const e of fs.readdirSync(d, { withFileTypes: true })) {
            ...
            if (e.isDirectory()) w(f); else if (e.name.endsWith('.prsm')) results.push(f);
        }} catch {}
    })(root);
    return results;
}
...
<div id="toolbar">
    <span class="title">PrSM Graph</span>
