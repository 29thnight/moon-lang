...
v1이 "작성 가능하고 Unity에 붙는 최소 제품"이라면, v2의 목표는 Moon을 중간 규모 Unity 프로젝트와 팀 단위 워크플로에 견딜 수 있는 도구 체계로 확장하는 것이다.
...
4. 런타임 오류, 콘솔 로그, 디버깅 흐름을 원본 `.mn` 기준으로 추적 가능하게 만들기
...
v2부터 `.mnproject`에 언어 버전과 기능 플래그를 명시한다.
...
- Unity `Packages/manifest.json` 또는 `.mnproject` capability에서 Input System 패키지 존재를 감지
...
- `SourceMapIndex`: `.mn`와 generated C# 위치 매핑

캐시 정책:

- 작업 디렉터리 아래 `.moon/cache` 사용
...
v2의 편집기 경험은 현재 확장의 shell-out 중심 모델을 넘어서야 한다. 설계는 `moonc lsp` 서브커맨드 또는 별도 `moonls` 바이너리 두 가지 중 하나로 수렴한다.

권장 방향:

- 배포 복잡도를 줄이기 위해 `moonc lsp` 우선
...
- Unity Console 예외와 컴파일 진단이 `.mn` 기준으로 안정적으로 역매핑됨
